...
> 말투, 승인 절차, 에이전트 호출 방식 같은 공통 규칙은 `CLAUDE.md` 의 공통 작업 규칙을 따른다.

1. **OWASP 체크리스트를 자의적으로 줄이지 않는다.** "이상 없음" 절도 반드시 포함한다.
2. **state 파일을 반드시 갱신한다.** 주기 상기의 근거가 된다.
3. **리포트를 파일로 저장하지 않는다.** 남겨야 하면 `tasks.md` 항목으로.
4. **메인 세션은 지휘만 한다.** 파일 식별과 맥락 수집과 대화를 맡고 분석은 에이전트가 한다.
