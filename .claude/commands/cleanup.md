...
1. **탐지와 삭제를 가른다.** 탐지 단계는 읽기 전용이고 삭제는 명시적 승인 후에만 한다.
2. **오탐을 전제한다.** 참조 없는 파일 탐지는 간이 분석이라 실제로 쓰이는 파일일 수 있다.
3. **일괄 자동 정리를 하지 않는다.** 여러 파일을 지울 때는 한 번 더 확인한다.
4. **보호 경로를 건드리지 않는다.** `.git`, `node_modules`, 빌드 산출물은 요청이 있어도 거부한다.
5. **state 를 기록한다.** 주기 상기의 근거가 된다.

`.git` 과 Node.js 가 필요하다.
